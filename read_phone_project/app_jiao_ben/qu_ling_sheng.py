...
class QuLingSheng(AppReadBase):
    ...
    def _read_issue_core(self, time1, time2):
        ...
        while time.time() - issue_time_start <= read_issue_time:
            ...
            #     time.sleep(random.random() + 1)
            # 按照设定的评论概率，随机评论
            # if self.pp.xpath('//*[@resource-id="com.zheyun.bumblebee:id/feed_video_comment"]').exists and \
            #         random.random() < self.probability_commit:
            #     self.click_random_position(self.pp.xpath('//*[@resource-id="com.zheyun.bumblebee:id/feed_video_comment"]')
            #                                .get().bounds)
            #     time.sleep(random.random() + 1)
